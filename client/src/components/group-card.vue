<template>
  <div
    class="group-card"
    @contextmenu.prevent="(admin)? cardEditable = true : false"
  >
    ...
    <md-card md-with-hover :class="{disabled}">
      ...
      <md-card-actions class="group-actions" v-if="!cardEditable">
          ...
          <div class="space" @click="goToCard"></div>
...
export default {
  ...
  computed: {
    cardTags(){
      return this.tags.filter(tag => {
        return this.boardTags.find(t => t._id === tag._id)
      }).map(tag => {
        return this.boardTags.find(t => t._id === tag._id)
      })
    },
  ...
  methods: {
   ...
   goToCard(){ //if disabled - forbidden message
     if(!this.cardEditable && !this.disabled)
       this.$router.push({
         name: 'Card',
         params: {
           id: this._id
